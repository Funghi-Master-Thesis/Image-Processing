...
def process_gray_image(img):
    ...
    # Apply Prewitt filter to compute gradients
    gradient_img = filters.prewitt(img)
    
    # Apply Otsu's thresholding to compute binary image
    threshold = threshold_otsu(gradient_img)
...
def capture_from_camera_and_show_images():
    ...
    while not stop:
        ...
        if process_rgb:
            ...
            size = 10
            new_image_gray = filters.median(new_image_gray, np.ones((size, size)))
